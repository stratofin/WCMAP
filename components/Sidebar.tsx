...
export default function Sidebar({ open, filters, onChange, total, filtered, mobileView }: SidebarProps) {
  const toggle = (key: keyof Omit<Filters, "search">) =>
    ...
    <aside style={mobileView ? mobileStyle : desktopStyle}>
      ...
      <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16, height: "100%", boxSizing: "border-box" }}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: "#6b7280" }}>
          顯示 <strong style={{ color: "#0D9488" }}>{filtered}</strong> / {total} 間廁所
        </p>

        {/* Search */}
        <input
          type="search"
          placeholder="🔍 搜尋名稱或行政區..."
        ...
        <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 12 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <LegendItem color="#0D9488" label="公廁標記" />
            <LegendItem color="#f59e0b" label="附近廁所（Near Me）" />
            <LegendItem color="#3b82f6" label="你的位置" />
          </div>
        </div>
